...
function render(){root.innerHTML=header()+`<main class="shell">${navigation()}${({overview,students:studentsView,rooms:roomView,sessions:sessionsView,assessment:assessmentView,reports:reportsView,audit:auditView})[tab]()}<div class="footer">資料儲存在本機 SQLite · 畫面每 15 秒更新</div></main>`+(modal?modalHtml():'');renderToast()}
